...
<div class="flex pl-0 p-4 space-x-3 items-center justify-between">
  ...
  {#if hasPermission("inventory.add_input")}
    <Button
      size="sm"
      color="blue"
      pill
      on:click={() => navigateTo("/inventory/input_form?type=new")}
...
>
  <TableHead class={"sticky top-0 w-full"}>
    ...
    <TableHeadCell scope="col" class={"text-center"}>Proveedor</TableHeadCell>
    <TableHeadCell scope="col" class={"text-center"}>No. Orden</TableHeadCell>
  ...
  <TableBody tableBodyClass={"divide-y min-h-full"}>
    {#each inputsObject.results as input, i}
      <TableBodyRow class="h-5">
        <TableBodyCell class={"w-[3%] p-2 text-center"}
          >{(i += 1)}</TableBodyCell
        >
        <TableBodyCell class="w-[25%] p-2 text-center"
          >{input.item?.name.toUpperCase()}</TableBodyCell
        >
        <TableBodyCell class="w-[25%] p-2 text-center"
          >{input.quantity}</TableBodyCell
        >
        <TableBodyCell class={"w-[10%] p-2 text-center"}
          >{$providers.find((e) => e.id === input.provider).name}</TableBodyCell
        >
        <TableBodyCell class={"w-[10%] p-2 text-center"}
          >{input.purchase_order ? input.purchase_order : ""}</TableBodyCell
        >
        <TableBodyCell class={"w-[12%] p-2 text-center"}
          >{new Date(input.date_of_entry).toLocaleString(
            "es-DO"
          )}</TableBodyCell
        >
        <TableBodyCell class={"w-[10%] p-2 text-center"}>
          <div class="flex justify-center">
            ...
            <Dropdown>
              <DropdownItem
                on:click={() =>
                  navigateTo(`inventory/input_form?type=update&id=${input.id}`)}
              >
                Editar</DropdownItem
              >
            </Dropdown>
