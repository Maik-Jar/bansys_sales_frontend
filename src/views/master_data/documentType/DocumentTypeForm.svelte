<script>
  ...
  const documentType = Object.seal({
    id: null,
    name: null,
    status: true,
  });
    ...
    if (isEditable) {
      ...
      fetch(
        urls.backendRoute +
          ...
          `?document_type_id=${currentRoute.queryParams.id}`,
        {
          headers: {
  ...
  function createDocumentType() {
    Swal.fire({
      title: "¿Quieres crear este nuevo tipo de documento?",
      showDenyButton: true,
      ...
      if (result.isConfirmed) {
        const token = JSON.parse(localStorage.getItem("token"));
        fetch(urls.backendRoute + urls.documentsTypesEndPoint, {
          method: "post",
          headers: {
          ...
          .then(async (res) => {
            if (res.ok) {
              ...
              Swal.fire("¡Nuevo tipo de documento creado!", "", "success");
              isEditable = true;
            } else {
...
>
  ...
  >
    <Heading tag="h4" class="w-1/2 {customColorsClassDark.label} self-end"
      >Código: <Span highlight>{documentType.id ? documentType.id : ""}</Span
      ></Heading
    >
  </div>
  ...
  >
    ...
    <Label class={customColorsClassDark.label}
      ...
      <Select
        ...
        bind:value={documentType.status}
        required
      >
...
<div class="flex space-x-3 mt-3">
  <Button
    color="dark"
    on:click={() => navigateTo("master_data/document_type_list")}>Volver</Button
  >
  {#if hasPermission("master_data.add_documenttype") || hasPermission("master_data.change_documenttype")}
    <Button color="green" type="submit" form="form_documentType">Guardar</Button
    >
  {/if}
</div>
