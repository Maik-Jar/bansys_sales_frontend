<script>
  ...
  let searchTerm = "";
  let paymentMethodObjects = {
    results: [],
  };
  let page = 1;

  function getPaymentMethods(page) {
  ...
  function previousPage() {
    if (paymentMethodObjects.previous) {
      page--;
      getPaymentMethods(page);
    }
  }
...
<div class="flex pl-0 p-4 space-x-3 items-center justify-between">
  ...
  {#if hasPermission("master_data.add_paymentmethod")}
    <Button
      ...
      color="blue"
      pill
      on:click={() => navigateTo("master_data/payment_method_form?type=new")}
      >Nuevo Método de pago</Button
    >
  {/if}
</div>
<Table
...
>
  ...
  <TableBody tableBodyClass={"divide-y min-h-full"}>
    {#each paymentMethodObjects.results as paymentMethod, i}
      <TableBodyRow class="h-5">
        <TableBodyCell class={"w-[3%] p-2 text-center"}
          >{(i += 1)}</TableBodyCell
        >
        <TableBodyCell class="w-[25%] p-2"
          >{paymentMethod.name.toUpperCase()}</TableBodyCell
        >
          ...
          <div class="flex justify-center">
            ...
            <Dropdown>
              <DropdownItem
                on:click={() =>
                  navigateTo(
                    `master_data/payment_method_form?type=update&id=${paymentMethod.id}`
                  )}
              >
                Editar</DropdownItem
              >
